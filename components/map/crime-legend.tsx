	...
	if (loading) {
		return (
			<div className="flex flex-col gap-1.5 pointer-events-auto">
				<div className="px-2 py-1.5 bg-white/80 dark:bg-[#0F172A]/60 border border-slate-200 dark:border-white/[0.05] rounded-lg shadow-sm dark:shadow-none">
					<span className="text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500">Crime Types</span>
				</div>
				{[1, 2, 3, 4, 5].map((i) => (
					<div key={i} className="h-9 w-[140px] rounded-lg bg-slate-200/60 dark:bg-white/[0.04] animate-pulse" />
				))}
			</div>
		);
	}
	...
	if (stats.length === 0) {
		return (
			<div className="flex flex-col gap-1.5 pointer-events-auto">
				...
				<div className="px-3 py-2 bg-white/80 dark:bg-[#0F172A]/60 border border-slate-200 dark:border-white/[0.05] rounded-lg shadow-sm dark:shadow-none">
					<span className="text-[10px] text-slate-500 dark:text-slate-400">No data available</span>
				</div>
			</div>
		);
	}
		...
		<div className="flex flex-col gap-1.5 pointer-events-auto">
			...
			<div className="px-2 py-1.5 bg-white/80 dark:bg-[#0F172A]/60 border border-slate-200 dark:border-white/[0.05] rounded-lg shadow-sm dark:shadow-none">
				<div className="flex items-center justify-between">
					<span className="text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500">Crime Types</span>
					<span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 tabular-nums">{formatNumber(total)} total</span>
				</div>
			</div>
				...
				return (
					<button
						...
						className={`flex items-center gap-2.5 h-9 pl-2 pr-3 rounded-lg border transition-all duration-300 cursor-pointer group ${
							isActive
								? "bg-slate-100 dark:bg-white/[0.08] border-slate-300 dark:border-white/[0.1] scale-105 shadow-sm dark:shadow-none"
								: isDimmed
									? "bg-white/60 dark:bg-[#0F172A]/60 border-slate-100 dark:border-white/[0.03] opacity-50 dark:opacity-40 scale-95"
									: "bg-white/80 dark:bg-[#0F172A]/60 border-slate-200 dark:border-white/[0.05] hover:bg-slate-50 dark:hover:bg-white/[0.06] hover:border-slate-300 dark:hover:border-white/[0.08] shadow-sm dark:shadow-none"
						}`}>
						{/* Count Badge */}
						<div
