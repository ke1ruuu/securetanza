...
export default function BarangayFilter() {
	const { selectedBarangay, setSelectedBarangay, filterOpen, setFilterOpen, searchQuery, setSearchQuery, filteredBarangays } = useMapContext();
				...
				<div className="p-3 border-b border-slate-100 dark:border-white/[0.04]">
					<div className="relative">
						...
						<input
							...
							autoFocus
							className="w-full bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] rounded-xl py-2.5 pl-9 pr-3 text-[13px] text-slate-900 dark:text-slate-200 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:bg-white dark:focus:bg-white/[0.04] focus:border-[#0EA5E9]/40 dark:focus:border-[#0EA5E9]/20 transition-colors"
							style={{ fontFamily: "var(--font-inter)" }}
						/>
					</div>
				</div>
							...
							<button
								...
								className={`block w-full text-left px-4 py-2.5 text-[13px] font-medium transition-all duration-150 ${
									selectedBarangay === name
										? "text-[#0EA5E9] bg-[#0EA5E9]/10 dark:bg-[#0EA5E9]/[0.08]"
