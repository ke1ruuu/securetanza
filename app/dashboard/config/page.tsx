...
function ConfigContent() {
      ...
      <div className={`flex flex-col h-screen ${theme === "dark" ? "bg-[#0f172a] text-white" : "bg-[#f1f5f9] text-slate-900"}`}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-slate-500 max-w-md mb-8">
            This module is reserved for Admin Operational Officers. 
            You do not have the necessary privileges to modify system configurations.
          </p>
        </div>
      </div>
    );
  }
  const [activeTab, setActiveTab] = useState<"settings" | "upload-logs">(
    "settings",
  );
  ...
  return (
    ...
    >
      <header
        className={`w-full bg-[#0F172A]/80 backdrop-blur-xl border-b border-white/[0.06] pointer-events-auto z-50`}
      >
        <div className="flex items-center h-16 px-8">
          <button
            onClick={() => router.back()}
      ...
      <div
        className={`flex items-center gap-1 px-8 py-4 border-b ${
          theme === "dark"
            ? "bg-[#0f172a]/80 border-white/[0.04]"
                ...
                <div className="flex justify-between items-start mb-8">
                  <div className="space-y-1">
                    <h2 className="text-2xl font-bold tracking-tight">
                      Privileged Personnel Authorization
                    </h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">
                      Manage administrative and operational access for
                      privileged users within the secure platform.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                  ...
                  {[1, 2, 3].map((i) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-lg ${theme === "dark" ? "bg-white/10" : "bg-slate-300"}`}
                        ></div>
                        <div className="flex-1">
                          <div
                            className={`h-8 w-16 rounded mb-2 ${theme === "dark" ? "bg-white/10" : "bg-slate-300"}`}
                          ></div>
                          <div
                            className={`h-4 w-24 rounded ${theme === "dark" ? "bg-white/10" : "bg-slate-300"}`}
                          ></div>
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-2xl font-bold text-slate-900 dark:text-white">
                          {logs
                            .reduce((sum, log) => sum + log.recordsImported, 0)
                            .toLocaleString()}
                        </div>
                        <div className="text-sm text-slate-500 dark:text-slate-400">
                          Total Records
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-2xl bg-[#0EA5E9]/10 flex items-center justify-center">
                {showCreateModal ? (
                  <UserPlus className="h-5 w-5 text-[#0EA5E9]" />
                ) : (
                  <Edit className="h-5 w-5 text-[#0EA5E9]" />
                )}
              </div>
            ...
            <form
              onSubmit={showCreateModal ? handleCreateUser : handleUpdateUser}
              className="space-y-4"
            >
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex p-1 bg-slate-950/50 rounded-xl border border-white/5">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                        ...
                        availablePermissions.find(p => p.id === formData.permissionIds[0])?.permissionName === 'admin_operational_officer'
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                        ...
                        availablePermissions.filter(p => formData.permissionIds.includes(p.id) && p.permissionName.startsWith('privileged_')).length > 0
                          ? "bg-[#0EA5E9] text-white shadow-lg"
                          : "text-slate-500 hover:text-slate-300"
                      }`}
                    >
                ...
                {(formData.permissionIds.length === 0 || 
                  availablePermissions.some(p => formData.permissionIds.includes(p.id) && p.permissionName.startsWith('privileged_'))) && (
                          ...
                          return (
                                ...
                                setFormData(prev => ({
                                  ...
                                  permissionIds: isSelected
                                    ...
                                    : [...prev.permissionIds.filter(id => {
                                        ...
                                        return perm && perm.permissionName !== 'admin_operational_officer';
                                      }), p.id]
                                }));
                              }}
                            ...
                            >
                              <div className="flex flex-col">
                                <span className={`text-[11px] font-bold uppercase tracking-tight ${isSelected ? "text-[#0EA5E9]" : "text-slate-400"}`}>
                                  {p.permissionName.replace('privileged_', '').replace('_view', '').replace(/_/g, ' ')}
                                </span>
