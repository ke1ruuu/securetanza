...
function CasesContent() {
  ...
  if (!user || (!user.permissions.includes("admin_operational_officer") && !user.permissions.includes("admin") && !user.permissions.includes("privileged_cases_view"))) {
    return (
      <div className={`flex flex-col h-screen ${theme === "dark" ? "bg-[#0f172a] text-white" : "bg-[#f1f5f9] text-slate-900"}`}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-slate-500 max-w-md mb-8">
            You do not have the necessary permissions to view the Cases Dashboard. 
      ...
      <div className={`flex items-center gap-4 px-6 py-3 border-b shrink-0 ${
        ...
        <span className={`text-sm font-medium ${
          ...
          {barangayName === "General Dashboard" 
            ? "All crime cases across Tanza, Cavite" 
            : `Crime cases in Brgy. ${barangayName}`}
        </span>
      </div>
